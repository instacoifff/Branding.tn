...
import { ArrowRight, CheckCircle, Star, Sparkles, PenTool, Image, Share2, Shield, TrendingUp, Zap } from "lucide-react";
  ...
  return (
    <div className="min-h-screen bg-background text-foreground overflow-x-hidden">
      ...
      <section ref={heroRef} className="relative min-h-screen flex flex-col items-center justify-center text-center px-6 pt-28 pb-20">
        ...
        <motion.div style={{ y: heroY, opacity: heroOpacity }} className="relative z-10 max-w-5xl mx-auto">
          ...
          <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5 }}
            className="text-muted-foreground text-lg sm:text-xl max-w-[600px] mx-auto leading-relaxed mb-6 font-light">
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.6 }}
            ...
            <span className="text-muted-foreground text-sm ml-2">4.9/5 from 120+ clients</span>
          </motion.div>
      ...
      <section className="py-12 border-y border-border overflow-hidden bg-muted/30">
        <p className="text-center text-muted-foreground/50 text-[11px] font-medium uppercase tracking-[0.2em] mb-8">
            ...
            {[...Array(2)].map((_, setIdx) => (
              <div key={setIdx} className="flex items-center gap-16 px-8">
                {["Decathlon", "Toyota", "AXA", "KPMG", "L'Oréal", "Nestlé", "Tunisair", "Ooredoo"].map((name) => (
                  <span key={`${setIdx}-${name}`} className="text-foreground/20 text-lg font-semibold tracking-tight whitespace-nowrap select-none">
                    {name}
                  </span>
                ))}
              </div>
      ...
      <section className="py-24">
        <div className="max-w-[1000px] mx-auto px-6">
          <motion.div variants={stagger} initial="hidden" whileInView="show" viewport={{ once: true, margin: "-100px" }}
            ...
            {[
              { value: "500+", label: "Projects shipped", accent: "group-hover:bg-blue-500/5 group-hover:border-blue-500/15" },
              { value: "98%", label: "Client satisfaction", accent: "group-hover:bg-emerald-500/5 group-hover:border-emerald-500/15" },
              { value: "120+", label: "Happy clients", accent: "group-hover:bg-violet-500/5 group-hover:border-violet-500/15" },
              { value: "7 days", label: "Avg. delivery", accent: "group-hover:bg-amber-500/5 group-hover:border-amber-500/15" },
            ].map((s) => (
              <motion.div key={s.value} variants={fadeUp}
                className={`group rounded-2xl border border-border p-6 text-center transition-all duration-300 bg-card ${s.accent}`}>
                <p className="text-3xl md:text-4xl font-bold text-foreground mb-1.5">{s.value}</p>
      ...
      <section className="py-24">
        <div className="max-w-[1100px] mx-auto px-6">
          <motion.div variants={fadeUp} initial="hidden" whileInView="show" viewport={{ once: true }} className="text-center mb-20">
            <p className="text-primary text-xs font-semibold uppercase tracking-[0.2em] mb-4">What We Create</p>
            <h2 className="text-4xl md:text-5xl font-semibold tracking-tight text-foreground mb-5" style={{ fontFamily: "'Outfit', sans-serif" }}>
                ...
                <div className={`grid ${i === 0 ? "lg:grid-cols-5" : "lg:grid-cols-2"} gap-0`}>
                  ...
                  <div className={`${i === 0 ? "lg:col-span-2" : ""} p-8 lg:p-10 flex flex-col justify-between`}>
                    <div>
                      <div className="flex items-center gap-3 mb-5">
                        <div className="w-10 h-10 rounded-xl bg-primary/10 border border-primary/15 flex items-center justify-center">
                          <svc.icon size={17} className="text-primary" />
                        </div>
                        <h3 className="text-xl font-semibold text-foreground">{svc.title}</h3>
                      </div>
                      <p className="text-muted-foreground text-sm leading-relaxed mb-6 font-light">{svc.desc}</p>
                      <div className="flex flex-wrap gap-2 mb-8">
                        {svc.tags.map((tag) => (
                          <span key={tag} className="text-[11px] px-2.5 py-1 rounded-full bg-muted border border-border text-muted-foreground flex items-center gap-1">
                            <CheckCircle size={9} className="text-emerald-500" /> {tag}
                          </span>
                        ))}
                      </div>
                    </div>
                    <div className="flex items-center justify-between">
                      <div>
                        <span className="text-muted-foreground text-xs uppercase tracking-wider">Starting at</span>
                        <p className="text-2xl font-bold text-foreground">{svc.price}</p>
                      </div>
                      <Link to="/builder">
      ...
      <section className="py-24 bg-muted/30">
        <div className="max-w-[1000px] mx-auto px-6">
          <motion.div variants={fadeUp} initial="hidden" whileInView="show" viewport={{ once: true }} className="text-center mb-20">
            <p className="text-primary text-xs font-semibold uppercase tracking-[0.2em] mb-4">How It Works</p>
            <h2 className="text-4xl md:text-5xl font-semibold tracking-tight text-foreground mb-5" style={{ fontFamily: "'Outfit', sans-serif" }}>
          ...
          <motion.div variants={stagger} initial="hidden" whileInView="show" viewport={{ once: true }}
            ...
            {PROCESS.map((p) => (
              <motion.div key={p.n} variants={fadeUp}
                className="relative rounded-2xl border border-border p-7 group hover:border-primary/20 transition-all duration-400 bg-card hover:shadow-card">
                <span className="text-[72px] font-black text-foreground/[0.04] absolute top-0 right-3 leading-none select-none group-hover:text-primary/[0.08] transition-colors duration-500">
                  {p.n}
                </span>
      ...
      <section className="py-24">
        <div className="max-w-[1000px] mx-auto px-6">
          <motion.div variants={fadeUp} initial="hidden" whileInView="show" viewport={{ once: true }} className="text-center mb-16">
            <p className="text-primary text-xs font-semibold uppercase tracking-[0.2em] mb-4">Why BrandingTN</p>
            <h2 className="text-4xl md:text-5xl font-semibold tracking-tight text-foreground" style={{ fontFamily: "'Outfit', sans-serif" }}>
          ...
          <motion.div variants={stagger} initial="hidden" whileInView="show" viewport={{ once: true }}
            ...
            {REASONS.map((r) => (
              <motion.div key={r.title} variants={fadeUp}
                className="rounded-2xl border border-border p-6 hover:border-primary/20 transition-all bg-card hover:shadow-card group">
                <span className="text-2xl mb-4 block group-hover:scale-110 transition-transform origin-left">{r.emoji}</span>
                <h3 className="text-[15px] font-semibold text-foreground mb-2">{r.title}</h3>
          ...
          <motion.div variants={fadeUp} initial="hidden" whileInView="show" viewport={{ once: true }}
            className="mt-8 rounded-2xl border border-emerald-500/15 bg-emerald-500/[0.03] p-6 flex flex-col sm:flex-row items-center gap-5 text-center sm:text-left">
            ...
            <div>
              ...
              <p className="text-muted-foreground text-sm font-light">Not thrilled with the result? We revise for free until you are — or full refund. Zero risk for you.</p>
            </div>
          </motion.div>
        </div>
      </section>
      ...
      <section className="py-24 bg-muted/30">
        <div className="max-w-[1000px] mx-auto px-6">
          <motion.div variants={fadeUp} initial="hidden" whileInView="show" viewport={{ once: true }} className="text-center mb-16">
            <p className="text-primary text-xs font-semibold uppercase tracking-[0.2em] mb-4">Client Results</p>
            <h2 className="text-4xl md:text-5xl font-semibold tracking-tight text-foreground" style={{ fontFamily: "'Outfit', sans-serif" }}>
          ...
          <motion.div variants={stagger} initial="hidden" whileInView="show" viewport={{ once: true }}
            className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {TESTIMONIALS.map((te) => (
              <motion.div key={te.name} variants={fadeUp}
                className="rounded-2xl border border-border p-7 flex flex-col gap-5 bg-card hover:border-primary/15 hover:shadow-card transition-all">
                <div className="flex gap-0.5">
                  {[...Array(5)].map((_, i) => <Star key={i} size={12} className="text-amber-400 fill-amber-400" />)}
                </div>
                <p className="text-muted-foreground text-sm leading-relaxed flex-1 font-light">"{te.quote}"</p>
                <span className="inline-flex items-center gap-1.5 text-[11px] font-medium text-emerald-600 bg-emerald-500/8 border border-emerald-500/15 px-2.5 py-1 rounded-full w-fit">
                  <TrendingUp size={10} /> {te.result}
                </span>
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-foreground text-sm font-medium">{te.name}</p>
      ...
      <section className="py-32 relative">
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
          <div className="blob-blue w-[600px] h-[400px]" />
        </div>
        <div className="max-w-3xl mx-auto px-6 text-center relative z-10">
          <motion.div variants={fadeUp} initial="hidden" whileInView="show" viewport={{ once: true }}>
            ...
            <h2 className="text-4xl md:text-[56px] font-semibold leading-[1.1] tracking-tight text-foreground mb-7" style={{ fontFamily: "'Outfit', sans-serif" }}>
              Your competitors already
              <br />
              <span className="headline-gradient">have a brand. Do you?</span>
            </h2>
            <p className="text-muted-foreground text-base max-w-md mx-auto mb-5 font-light">
              Real-time pricing. Interactive project builder. First concepts in 72 hours. Only 30% deposit to start.
            </p>
            <p className="text-muted-foreground/50 text-xs mb-12">
